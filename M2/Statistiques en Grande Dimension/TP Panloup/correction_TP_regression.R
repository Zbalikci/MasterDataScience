...
##Q5
v=coef(LASSO_ex1,s=0.72070)
v[1:30]
sum(v==0)
##Q6 : fonction predict
## Train error
Ypred=predict(LASSO_ex1,x, s=0.15)
TrainMSE=1/n*sum((Ypred-y)^2)
TrainMSE
ValSetX=matrix(rnorm(n*p),nr=n)
Ytest=simuy(n,p,theta,sigma,ValSetX)
Ypred=predict(LASSO_ex1, ValSetX, s=0.3)
ErreurPredCar=1/n*(sum((Ypred-Ytest)^2))
ErreurPredCar
...
plot(Ridge_ex1)
dev.off()
cvRidge_ex1=cv.glmnet(x,y,alpha=0,lambda=seq(0.01,5,by=0.1))
### A revoir...
...
leukemia_small= read.csv("/users/2024/ds2/122003362/Téléchargements/leukemia_small.csv")

#  gènes = matrice X et Y réponse
X = as.matrix(t(leukemia_small))
Y = c(rep(1,27),rep(0,11),rep(1,11),rep(0,5),1,1,0,0,1,rep(0,7),rep(1,6))
...
hist(X[,147],breaks=30, probability = TRUE, main = "Histogram of the genes log-expression", xlab="Genes log-expression")
...
png(file = "PCA_leukemiabis.png", width = 800, height = 700)
par(mfrow=c(1,3))
...
png(file = "validation_croisee_leukemiabis.png", width = 800, height = 700)
par(mfrow=c(3,2))
plot(LambdaLasso)
plot(LambdaRidge)
plot(LambdaEN1)
...
sum(abs(ValSetY - ValSetYPredEN3))/24
