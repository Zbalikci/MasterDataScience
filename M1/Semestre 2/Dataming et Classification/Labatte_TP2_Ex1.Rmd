...
Les variables légumes, fruit viande et volaile sont positivement corrélé entre eux
Le lait est positivement corrélé au lait, et la viande est très peu corrélé avec les autres variables
...
On peut choisir de retenir que 2 axes variables (règle de Kaiser et méthode du coude, et on a 88% du jeu de donnés ce qui est largement suffisant)
On pourra aussi décider de prendre l'axe 3 (même s'il la contribution n'est que de 9%) mais retenir les axes 4,5,6 et 7 sera inutile au vue de peu d'informations qu'il rapporte
```{r}
csp$eig
...
```{r}
plot.PCA(csp,choix="ind",select="cos2 0.5") 
corrplot(csp$ind$contrib,is.corr = FALSE)
fviz_contrib(csp,"ind",axes = 2)
plot.PCA(csp,choix="ind",select="contrib 10 ") 
csp$ind
```
